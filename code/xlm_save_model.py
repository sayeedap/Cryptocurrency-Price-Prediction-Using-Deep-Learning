...
hist = pd.read_csv("xlm.csv", usecols =[0,1,2,3,4],names=['time', 'low', 'high', 'open', 'close',])
...
train, test,say_test = train_test_split(hist, test_size=0.4)
...
def prepare_data(df, target_col, window_len=10, zero_base=True, test_size=0.4):
...
#say_test_data.head()
saymodel = build_lstm_model(X_train, output_size=1, neurons=lstm_neurons, dropout=dropout, loss=loss,optimizer=optimizer)
saymodel.save("modelxlm.h5")
