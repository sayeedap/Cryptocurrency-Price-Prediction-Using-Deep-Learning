...
hist = pd.read_csv("ltc.csv", usecols =[0,1,2,3,4],names=['time', 'low', 'high', 'open', 'close',])
...
def d7 ():
        ...
        plt.plot(hist[target_col][start:end], 'r', label = 'Closing price')
...
def m1 ():
        ...
        #print('start=',start,' end=',end,'total',len(hist))
...
def all ():
        ...
        plt.plot(hist[target_col][:end], 'r', label = 'Closing price')
        plt.legend(loc = 'upper right')
        plt.xlabel('Days') 
        # naming the y axis 
        plt.ylabel('price(USD)') 
